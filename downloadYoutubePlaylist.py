...
url = "https://www.youtube.com/playlist?list=PLAwxTw4SYaPlBwWBVoIWKfSWxzuhfSW-D";
source_code = requests.get(url);
plain_text = source_code.text;
soup = BeautifulSoup(plain_text , 'html.parser');
save_path =  'N:\Videos\Demo\Demo';
#title = soup.find('h1' ,attrs={'class':'pl-header-title'}).text;
#playlist_name = title.strip();
#save_path = os.path.normpath(save_path + '/'+ playlist_name);
#print(save_path);
os.mkdir(save_path);
